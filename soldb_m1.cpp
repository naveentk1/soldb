...
static bool detect_cpu_features(Soldb *db) {
  ...
  if (sysctlbyname("hw.optional.arm.FEAT_CRC32", &ret, &size, NULL, 0) == 0 &&
      ret == 1) {
    db->has_crc32 = true;
  ...
  unsigned long mem = 0; // changed type from uint64_t
  size = sizeof(mem);
...
static bool soldb_find(Soldb *db, uint32_t hash, const char *key,
                       size_t key_len, size_t *idx_out, bool *found_out) {
  ...
  while (probes < 128) {
    ...
    uint16_t stored_key_len =
        atomic_load_explicit(&e->key_len, memory_order_relaxed);
...
static void soldb_resize(Soldb *db, size_t new_size) {
  ...
  HashEntry *new_table =
      (HashEntry *)aligned_alloc(128, new_size * sizeof(HashEntry));
  if (!new_table)
    return;
  ...
  for (size_t i = 0; i < old_size; i++) {
    ...
    if (atomic_load_explicit(&old_entry->occupied, memory_order_relaxed)) {
      uint32_t hash =
          atomic_load_explicit(&old_entry->hash, memory_order_relaxed);
      uint16_t klen =
          atomic_load_explicit(&old_entry->key_len, memory_order_relaxed);
      uint16_t vlen =
          atomic_load_explicit(&old_entry->value_len, memory_order_relaxed);
      ...
      while (probes < 128) {
        ...
        if (!atomic_load_explicit(&new_entry->occupied, memory_order_relaxed)) {
          ...
          atomic_store_explicit(&new_entry->key_len, klen,
                                memory_order_relaxed);
          atomic_store_explicit(&new_entry->value_len, vlen,
                                memory_order_relaxed);
          atomic_store_explicit(&new_entry->occupied, true,
                                memory_order_relaxed);
          break;
        }
...
static bool soldb_get(Soldb *db, const char *key, size_t key_len,
                      char *value_out, size_t *value_len_out);

static bool soldb_set(Soldb *db, const char *key, size_t key_len,
                      const char *value, size_t value_len) {
  if (key_len == 0 || key_len > MAX_KEY_SIZE)
    return false;
  if (value_len == 0 || value_len > MAX_VALUE_SIZE)
    return false;
  if (key_len + value_len > HASH_ENTRY_DATA_SIZE)
    return false;
  ...
  if (!soldb_find(db, hash, key, key_len, &idx, &found))
    return false;
  ...
  atomic_store_explicit(&e->value_len, (uint16_t)value_len,
                        memory_order_relaxed);
...
static int load_csv(const char *filename, CSVEntry **entries_out,
                    size_t *count_out) {
...
static void init_panel(Panel *p, const char *title, int color, int row, int col,
                       int h, int w) {
...
static void show_buffer(WINDOW *win, char buf[][128], int count,
                        int start_row) {
  if (!win)
    return;
    ...
    for (int batch = 0; batch < 5; batch++) {
      for (size_t i = 0; i < 50000 && ctx->pos < count; i++) {
        ...
        if (i % display_interval == 0) {
          ...
          snprintf(ctx->db_buf[buf_count], sizeof(ctx->db_buf[0]), "%s -> %s",
                   e->key, e->value);
        ...
        if (i % 50 == 0 && ctx->pos > 0) {
          ...
          if (i % display_interval == 0) {
            snprintf(ctx->exists_buf[buf_count % 20],
                     sizeof(ctx->exists_buf[0]), "EXISTS: %s = %s", ge->key,
                     ex ? "YES" : "NO");
            buf_count = (buf_count + 1) % 20;
          }
          ...
          if (neg_idx < count) {
            soldb_exists(db, entries[neg_idx].key,
                         strlen(entries[neg_idx].key));
          }
        }
...
static void run_tui(TUIContext *ctx) {
  ...
  while (*ctx->running) {
    ...
    mvwprintw(rate_win, 5, 1, "TOTAL: %6lu/s",
              (unsigned long)(ops + ops_get + ops_exists + ops_del));
    ...
    mvprintw(R - 1, 0,
             "SET:%lu GET:%lu EXISTS:%lu DEL:%lu | DB:%zu/%zu | VFY:%lu/%lu | "
             "%02ds | %s",
             (unsigned long)total_sets, (unsigned long)total_gets,
             (unsigned long)total_exists, (unsigned long)total_dels, db_count,
             db_capacity, verify_ok, verify_fail, elapsed,
